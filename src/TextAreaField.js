...
function Text({ disabled, error, errorMessage, helperText, inputRef, label, name, onChange, placeholder, readOnly, showInlineError, value = '', ...props }) {
    ...
    return (React.createElement(View, { style: {
        ...
        React.createElement(TextInput, { mode: "flat", disabled: disabled || readOnly, multiline: true, error: !!error, label: `${label}${props.required ? ' *' : ''}`, dense: true, onChangeText: (text) => disabled || onChange(text), placeholder: placeholder, ref: inputRef, value: value, style: { backgroundColor: 'transparent' }, underlineStyle: {
                width: '90%',
                height: 1.5,
